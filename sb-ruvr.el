;;; sb-ruvr.el --- shimbun backend for japanese.ruvr.ru -*- coding: iso-2022-7bit; -*-
;; Modified from sb-yomiuri.el

;; Below is the original copyright notice of sb-yomiuri.el.
...
(luna-define-class shimbun-ruvr
		   (shimbun-japanese-newspaper shimbun) ())

(defvar shimbun-ruvr-top-level-domain "japanese.ruvr.ru"
  "Name of the top level domain for the ruvr On-line.")

(defvar shimbun-ruvr-url
  (concat "http://" shimbun-ruvr-top-level-domain "/")
  "Name of the parent url.")

(defvar shimbun-ruvr-group-table
  (let* ((s0 "[\t\n ]*")
	 (s1 "[\t\n ]+")
	 (default
	   (list
    ...
    `(("atmoney" "マネー・経済" "news/" ,@default) ;; business
      ...
      ("kyoiku" "教育" ""
       ...
       1 2 4 3 5 6 7)
      ("national" "社会" "" ,@default)
      ("obit" "おくやみ" "http://www.ruvr.co.jp/national/obit/"
       ,(concat
	 "<a" s1 "href=\"/"
	 ;; 1. url
	 "\\(national/%s/news/"
...
(defvar shimbun-ruvr-content-start
  "\n<div class=\"doc_annotation\">")

(defvar shimbun-ruvr-content-end
  "\\(\\cj\\|</p>\\)</div>")

(defvar shimbun-ruvr-x-face-alist
...
(defvar shimbun-ruvr-expiration-days 7)

(luna-define-method initialize-instance :after ((shimbun shimbun-ruvr)
						 &rest init-args)
  (shimbun-set-server-name-internal shimbun "VOR")
  (shimbun-set-from-address-internal shimbun "nobody@example.com")
  ;; To share class variables between `shimbun-ruvr' and its
  ;; successor `shimbun-ruvr-html'.
  (shimbun-set-x-face-alist-internal shimbun shimbun-ruvr-x-face-alist)
  (shimbun-set-expiration-days-internal shimbun
					shimbun-ruvr-expiration-days)
  shimbun)

(luna-define-method shimbun-groups ((shimbun shimbun-ruvr))
  (mapcar 'car shimbun-ruvr-group-table))

(luna-define-method shimbun-current-group-name ((shimbun shimbun-ruvr))
  (nth 1 (assoc (shimbun-current-group-internal shimbun)
		shimbun-ruvr-group-table)))

(luna-define-method shimbun-index-url ((shimbun shimbun-ruvr))
  (let* ((group (shimbun-current-group-internal shimbun))
	 (index (nth 2 (assoc group shimbun-ruvr-group-table))))
    (if (string-match "\\`http:" index)
	index
      (concat shimbun-ruvr-url group "/" index))))

(defun shimbun-ruvr-japanese-string-to-number (string)
...
(defun shimbun-ruvr-get-top-header (group from)
  ...
  (when (re-search-forward
	 (format
	  ...
	  group)
	 nil t)
    (let* ((url (shimbun-expand-url (match-string 1) shimbun-ruvr-url))
	   (subgroup (match-string 2))
	   (id (concat "<" (match-string 3) "." (match-string 5)
		       "@" (when subgroup
			     (concat subgroup "."))
		       group "." shimbun-ruvr-top-level-domain ">"))
...
(defun shimbun-ruvr-get-headers (shimbun)
  ...
  (let ((group (shimbun-current-group-internal shimbun))
    ...
    (when (member group '("atmoney" "entertainment" "national" "politics"
      ...
      (setq headers (shimbun-ruvr-get-top-header group from)))
    (setq regexp (assoc group shimbun-ruvr-group-table)
    ...
    (while (re-search-forward regexp nil t)
      (setq subject (match-string (nth 4 numbers))
	    month (if (nth 8 numbers)
		      (shimbun-ruvr-japanese-string-to-number
...
(luna-define-method shimbun-get-headers ((shimbun shimbun-ruvr)
					 &optional range)
  (shimbun-ruvr-get-headers shimbun))

(defun shimbun-ruvr-prepare-article (shimbun header)
  (shimbun-strip-cr)
  (goto-char (point-min))
  (let ((case-fold-search t)
	start)
    (when (and (re-search-forward shimbun-ruvr-content-start nil t)
	       (progn
		 (setq start (match-end 0))
		 (re-search-forward shimbun-ruvr-content-end nil t)))
...
(luna-define-method shimbun-make-contents :before ((shimbun shimbun-ruvr)
						   header)
  (shimbun-ruvr-prepare-article shimbun header))

(provide 'sb-ruvr)

;;; sb-ruvr.el ends here
