;;; sb-cri.el --- shimbun backend for www.yomiuri.co.jp -*- coding: iso-2022-7bit; -*-
...
(luna-define-class shimbun-cri
		   (shimbun-japanese-newspaper shimbun-text) ())

(defvar shimbun-cri-top-level-domain "japanese.cri.cn"
  "Name of the top level domain for the 中国国際放送局.")

(defvar shimbun-cri-url
  (concat "http://" shimbun-cri-top-level-domain "/")
  "Name of the parent url.")

(defvar shimbun-cri-group-table
  (let* ((s0 "[\t\n ]*")
	 (s1 "[\t\n ]+")
	 (default
	   (list
	    (concat
	     ...
	     "[^\'\"]+\\)[\'\"][^>]+>" s0
	     ;; 6. IMG or FONT
	     "\\(<[^>]*>\\)?" s0 ;IMG ない時もあるかもしれないので。
	     ;; 7. subject
	     "\\([^<]+\\)"
	     s0 "\\(<[^a][^>]+>\\)*</a>")
	    1 5 5 2 7 3 4 nil)))
    `(("news" "ニュース" "news.htm" ,@default)
      ("economy" "暮らし" "economy.htm" ,@default)
      ("travel" "旅行・文化" "travel.htm" ,@default)
      ("ent" "エンタメ" "ent.htm" ,@default)
      ("china" "国際・交流" "china.htm" ,@default)
      ("special" "特別企画" "special.htm" ,@default)
      ))
...
(defvar shimbun-cri-content-start
  "\n<!--// article_start //-->\n\\|\n<!--  honbun start  -->\n")

(defvar shimbun-cri-content-end
  "\n<!--// article_end //-->\n\\|\n<!--  honbun end  -->\n")

(defvar shimbun-cri-x-face-alist
...
(defvar shimbun-cri-expiration-days 7)

(luna-define-method initialize-instance :after ((shimbun shimbun-cri)
						 &rest init-args)
  (shimbun-set-server-name-internal shimbun "cri")
  (shimbun-set-from-address-internal shimbun "nobody@example.com")
  ;; To share class variables between `shimbun-cri' and its
  ;; successor `shimbun-cri-html'.
  (shimbun-set-x-face-alist-internal shimbun shimbun-cri-x-face-alist)
  (shimbun-set-expiration-days-internal shimbun
					shimbun-cri-expiration-days)
  shimbun)

(luna-define-method shimbun-groups ((shimbun shimbun-cri))
  (mapcar 'car shimbun-cri-group-table))

(luna-define-method shimbun-current-group-name ((shimbun shimbun-cri))
  (nth 1 (assoc (shimbun-current-group-internal shimbun)
		shimbun-cri-group-table)))

(luna-define-method shimbun-index-url ((shimbun shimbun-cri))
  (let* ((group (shimbun-current-group-internal shimbun))
	 (index (nth 2 (assoc group shimbun-cri-group-table))))
    (if (string-match "\\`http:" index)
	index
      (concat shimbun-cri-url index))))

(defun shimbun-cri-japanese-string-to-number (string)
...
(defun shimbun-cri-get-top-header (group from)
  ...
  (when (re-search-forward
	 (format
	  ...
	  group)
	 nil t)
    (let* ((url (shimbun-expand-url (match-string 1) shimbun-cri-url))
	   (subgroup (match-string 2))
	   (id (concat "<" (match-string 3) "." (match-string 5)
		       "@" (when subgroup
			     (concat subgroup "."))
		       group "." shimbun-cri-top-level-domain ">"))
...
(defun shimbun-cri-get-headers (shimbun)
  ...
  (let ((group (shimbun-current-group-internal shimbun))
    ...
    (when (member group '("atmoney" "entertainment" "national" "politics"
      ...
      (setq headers (shimbun-cri-get-top-header group from)))
    (setq regexp (assoc group shimbun-cri-group-table)
    ...
    (while (re-search-forward regexp nil t)
      (setq subject (match-string (nth 4 numbers))
	    month (if (nth 8 numbers)
		      (shimbun-cri-japanese-string-to-number
...
(luna-define-method shimbun-get-headers ((shimbun shimbun-cri)
					 &optional range)
  (shimbun-cri-get-headers shimbun))

(defun shimbun-cri-prepare-article (shimbun header)
  (shimbun-strip-cr)
  (goto-char (point-min))
  (let ((case-fold-search t)
	start)
    (when (and (re-search-forward shimbun-cri-content-start nil t)
	       (progn
		 (setq start (match-end 0))
		 (re-search-forward shimbun-cri-content-end nil t)))
...
(luna-define-method shimbun-make-contents :before ((shimbun shimbun-cri)
						   header)
  (shimbun-cri-prepare-article shimbun header))
